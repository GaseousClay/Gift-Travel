...
pct <- function(x, y) {
      diff <- y - x
      pct.change <- 100 * diff / x
      return(pct.change)
}

setwd("/Users/philipmoniz/Dropbox/Gift Travel/Data")
...
extremitydata <- data.frame(extreme = giftdata$extreme, democrat = giftdata$democrat, winpct = mean(giftdata$winpct), seniority = mean(giftdata$seniority), chairman = 0, committee = 0, party.leader = 0, fourteenth = 0, thirteenth = 0, twelfth = 0, eleventh = 0)

extremitydataD1 <- filter(extremitydata, democrat == 1)

extremitydataR1 <- filter(extremitydata, democrat == 0)
...
rankingmemdata <- data.frame(extreme = mean(giftdata$extreme), democrat = giftdata$democrat, winpct = mean(giftdata$winpct), seniority = mean(giftdata$seniority), chairman = 0, committee = giftdata$committee, party.leader = 0, fourteenth = 0, thirteenth = 0, twelfth = 0, eleventh = 0)

rankingmemdataD1 <- filter(rankingmemdata, democrat == 1)

rankingmemdataR1 <- filter(rankingmemdata, democrat == 0)
...
extremitydataR1$phat <- predict(finalmodel, extremitydataR1, type = "response")
...
rankingmemdataR1$phat <- predict(finalmodel, rankingmemdataR1, type = "response")
...
summary(extremitydataR1$extreme)
summary(extremitydataR1$phat)
...
summary(rankingmemdataD1$phat)
summary(rankingmemdataR1$phat)
...
files <- read.csv(file = "/Users/philipmoniz/Documents/Scholastic Scribblings/Ph.D./2016-17 First Year/Spring 2017/Congress/Gift Travel paper/Gift-Travel/110-4GiftTravelFilings.csv",
